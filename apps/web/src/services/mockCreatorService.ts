...
async function mockResponse<T>(data: T): Promise<ApiEnvelope<T>> {
  return { data };
}

export const creatorService = {
  getDashboard: () => mockResponse<DashboardSummary>({ creator, stream }),
  getCommunity: () => mockResponse<CommunitySummary>({ recentActivity: activities, followers, subscribers }),
  getGrowth: () => mockResponse<GrowthSummary>({ points: growthPoints }),
  getHealthScore: () => mockResponse<HealthScore>(healthScore)
};
